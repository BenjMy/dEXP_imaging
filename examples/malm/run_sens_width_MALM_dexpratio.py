...
filenames = ['MSoilR1000AnoR1Z-13.75W5H2.5L5S0Noise5',
              'MSoilR1000AnoR1Z-13.75W15H2.5L5S0Noise5',
              'MSoilR1000AnoR1Z-13.75W25H2.5L5S0Noise5',
              'MSoilR1000AnoR1Z-13.75W35H2.5L5S0Noise5']
...
plt, cmap = pEXP.plot_xy(MESHratio[i],scaled=scl, label=LABELratio[i],
              markerMax=True,p1p2=np.array([p1,p2]),
              ax=ax, Xaxis=x_axis) 
...
ax.set_aspect(aspect=5)
# plt.annotate(r'$\rho$=' + str(CTm[i]),[(x1 + x2)/2, -(z1+z2)/2+3])
ax.set_title(r'$width$:35m')
ax.set_ylabel(r'')

plt.tight_layout(pad=0.2, h_pad=-8.2)
plt.savefig('dexpratio_width.png', dpi=450)
