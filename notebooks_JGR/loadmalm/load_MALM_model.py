...
def load_MALM_Landfill_model(path, filename, shape=None, field=True, interp=True, radius=20):
    ...
    if field==True:
        ...
        x, y, z, U_raw = np.loadtxt(path + 'uz0_grid.dat', unpack=True)
        # x , y, z = mesh_xyz
        # ------------------------------- Plot the data
        ax = plt.figure()
    ...
    return coord_xyz, coord_xyz_int, U, coords_liner, shape, max_elevation, p, data_struct, ax
