...
def find_peaks_U(upwc, widths, max_distances=None, gap_thresh=None,
               min_length=None, min_snr=1, noise_perc=1):
    """
    ...
    Parameters
    ----------
    upwc: matrice, upward continuation field
    widths: 1-D sequence
    ...
    1. Identify "ridge lines" in the cwt matrix. These are relative maxima
    at each row, connected across adjacent rows. See identify_ridge_lines
    2. Filter the ridge_lines using filter_ridge_lines.
    ...
    ridge_lines = _identify_ridge_lines(upwc, max_distances, gap_thresh)
    
    # print(ridge_lines[0][0])
    for rl in range(len(ridge_lines)):
        plt.plot(ridge_lines[rl][1],ridge_lines[rl][0],color='red')
        
    
    filtered = _filter_ridge_lines(upwc, ridge_lines, min_length=min_length,
