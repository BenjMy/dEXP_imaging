...
path =  'E:/Padova/Experiments/GC_2019_Landfill_Porto_MALM/Test/phNO/'
dataset = MALM.load_MALM_LandfillPorto(path=path, 
                                    filename=file,
                                    shape = (300,300),
                                    field=True,
                                    interp = interp,
...
out_r = MALM_pm.load_MALM_Porto_real(MainPath + '/malm_models/',
                          MainPath + './malm_models/XYObs_real_f_m3.txt',
                          shape=(100,100),
                          radius=200,
                          rcor=10,
...
U_af, p_af, bool_abovef, U_bf, p_bf = MALM.isabove(xf, yf, uf, 
                                  np.array(p1),np.array(p2))
# Compute p1 and p2 line equation ax + by + c = 0
af, bf, cf = MALM.slope(p1,p2)

# Mirror points with respect to p1p2 line
Umirrorf, xy_mirrorf = MALM.mirrorU_alongLine(U_af,p_af,bool_abovef,af,bf,cf)
...
xf_r, yf_r = MALM_pm.rotate_60(origin, point_torotate, angle_deg=rot, showfig=False)
Xfs = xf_r-min(xp_r)
Yfs = yf_r-min(yp_r)
