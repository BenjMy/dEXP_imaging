...
for i, nl in enumerate(depths):
    plt.scatter(
        RII_0[:, :][i][1],
        nl * np.ones(len(RII_0[:, :][i][1])),
        color="red",
        label="RII 0",
    )
    plt.scatter(
        RI_0[:, :][i][1],
        nl * np.ones(len(RI_0[:, :][i][1])),
        color="green",
        label="RI 0",
    )
    plt.scatter(
        RIII_0[:, :][i][1],
        nl * np.ones(len(RIII_0[:, :][i][1])),
        color="blue",
        label="RI 0",
    )
...
for i, nl in enumerate(depths):
    plt.scatter(
        RI_minmax[:, :][i][1],
        nl * np.ones(len(RI_minmax[:, :][i][1])),
        color="red",
        label="RII 0",
    )
    plt.scatter(
        RII_minmax[:, :][i][1],
        nl * np.ones(len(RII_minmax[:, :][i][1])),
        color="green",
        label="RI 0",
    )
    plt.scatter(
        RIII_minmax[i][1],
        nl * np.ones(len(RIII_minmax[:, :][i][1])),
        color="blue",
        label="RI 0",
    )
    plt.ylim(min(depths), max(depths))
...
x_r, z_r, d_r, up_f_Centralridge = gridder.profile(
    Xlin, Ylin, D, [0, 6], [0, 10.5], 1000
)
x_r, z_r, d_r, up_f_d1_Centralridge = gridder.profile(
    Xlin, Ylin, D1, [0, 6], [0, 10.5], 1000
)
x_r, z_r, d_r, up_f_d2_Centralridge = gridder.profile(
    Xlin, Ylin, D2, [0, 6], [0, 10.5], 1000
)
...
plt.legend(
    [
        "f",
        r"$f_{1}=\frac{\partial u}{\partial z}$",
        r"$f_{2}=\frac{\partial^{2} u}{\partial^{2} z}$",
    ]
)
...
plt.subplot(1, 3, 1)
plt.plot(x_fit, f(x_fit, *popt), "g--", label="fit")
...
plt.subplot(1, 3, 2)
plt.plot(x_fit, f(x_fit, *poptd1), "g--", label="fit")
...
plt.subplot(1, 3, 3)
plt.plot(x_fit, f(x_fit, *poptd2), "g--", label="fit")
