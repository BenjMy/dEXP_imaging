...
def cor_field_B(x, y, z, u, B, rho=100, **kwargs):
    ...
    * rho : int
    ...
    num = rho * I
    dist = np.sqrt(
        (x - B[0]) ** 2 + (y - B[1]) ** 2 + (z - B[2]) ** 2
    )  # distance between B and potential electrodes
    den = 2 * math.pi * dist
    u_B = num / den
...
def load_surfer(fname, dtype="float64"):
    ...
    * data : dict
    ...
    with open(fname) as input_file:
        ...
        assert np.allclose(dmin, field.min()), err_msg.format("Min", dmin, field.min())
        assert np.allclose(dmax, field.max()), err_msg.format("Max", dmax, field.max())
